...
int main()
{
    ...
    for (int i = 1; i <= n; ++i)
    {
        cout << "Stu " << i << " pref list: ";
        for (auto j : stu_rank[i])
            cout << j << " ";
        cout << "\n";
    }
    for (int i = 1; i <= n; ++i)
    {
        ...
        {
            if (stu_pos[i] != j &&
                count(stu_rank[i].begin(), stu_rank[i].end(), j) > 0 &&
                ((col_queue[j].size() < col_cap[j] && col_eval[j][i] > 0) ||
                 col_eval[j][i] > col_eval[j][col_queue[j].front()]))
            {
                cout << "Unmatched pair :" << i << ", " << j << "\n";
                return -1;
            }
        }
    }
