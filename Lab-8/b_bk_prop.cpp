        ...
        for (int b = 0; b <= n && b <= a; ++b)
        {
            for (int c = 0; c <= k; ++c)
            {
                for (int d = 0; d <= k; ++d)
                {
                    ...
                    {
                        if (b < n && c < k)
                            dp[a + 1][b + 1][c + 1][max(d - 1, 0)] = (dp[a + 1][b + 1][c + 1][max(d - 1, 0)] + dp[a][b][c][d]) % mod;
                        if (a - b < m && d < k)
                            dp[a + 1][b][max(c - 1, 0)][d + 1] = (dp[a + 1][b][max(c - 1, 0)][d + 1] + dp[a][b][c][d]) % mod;
                    }
                }
            }
        }
